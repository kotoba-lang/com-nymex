(ns nymex.main
  "Kotodama WASM entrypoint for the Nymex clean-room actor (L4) — Clojure port.
  ...
  facts (`nymex.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"nymex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nymex-compat" :tier tier :entities entities} 200])
